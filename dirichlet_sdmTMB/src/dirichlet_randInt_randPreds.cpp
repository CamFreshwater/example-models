...
Type objective_function<Type>::operator() () 
{
  ...
  matrix<Type> pred_eff_re(n_levels, n_cat);    //pred effects on log scale including REs
  ...
  if (re_predictions) {
    for (int m = 0; m < n_levels; m++) {
      for(int k = 0; k < n_cat; k++) {
        pred_eff_re(m, k) = pred_eff(m, k) + z_rfac(pred_factor2k_i(m));
      }
    }
    pred_gamma = exp(pred_eff_re.array());
    
    REPORT(pred_eff_re);
    ADREPORT(pred_eff_re);
  } else {
