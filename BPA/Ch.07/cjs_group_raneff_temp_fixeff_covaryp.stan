...
// Identical to cjs_group_raneff.stan but with time-varying effects for phi and p, as well as covariance among phi and p
...
parameters {
  ...
  vector<lower=0>[2] sigma_group;  // SD among groups; length two because two 
                                   // parameters
  corr_matrix[2] Rho;                  / correlation matrix for sigmas
}
...
model {
...

  {
    vector[2] YY[g];
    vector[2] MU;
    MU = [mean_beta_phi, mean_beta_p]';
    for (gg in 1:g) {
      YY[gg] = [beta_phi[gg], beta_p[gg]]';  
    }
    YY ~ multi_normal(MU, quad_form_diag(Rho, sigma_group));
  }
