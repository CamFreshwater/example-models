...
// Identical to cjs_add except p has random effects by group
...
parameters {
  ...
  vector<lower=0,upper=1>[g] mean_p;     // Mean group-spec. recap.
  // real<lower=0,upper=1> p_g[g];          // Group-spec. recapture
  matrix[n_occ_minus_1, g] eta_phi;
  cov_matrix[g] Sigma_phi;
  matrix[n_occ_minus_1, g] eta_p;
  cov_matrix[g] Sigma_p;
}
...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    for (t in first[i]:n_occ_minus_1) {
      ...
      p[i, t] = inv_logit(eta_p[t, group[i]]);
    }
  }
...
model {
  ...
  Sigma_phi ~ inv_wishart(df, R);
  Sigma_p ~ inv_wishart(df, R);

  for (t in 1:n_occ_minus_1) {
    eta_phi[t] ~ multi_normal(mu_phi, Sigma_phi);
    eta_p[t] ~ multi_normal(mu_p, Sigma_p);
  }
